...
export function ChatPage() {
  ...
  const receiveMessages = () => {
    ...
    socketProvided?.socket.on("server-message", (e) => {
      setMessages((prevMessages) => [...prevMessages, {type:"received", messageText: e}])
    })
  }
  const sendMessage = (textToSend:string) => {
    socketProvided?.socket.emit("mensaje", textToSend)
    setMessages((prevMessages) => [...prevMessages, {type:"sended", messageText: textToSend}])
  }
  ...
  return (
    ...
    <UserSection/>
      ...
      <div className="flex flex-col gap-2 h-full w-full">
        <ChatBodySection>
          {messages.length > 0 && messages.map((message, iter) => 
            (message.type === "received" ? <ReceivedMessage key={iter}>{message.messageText}</ReceivedMessage> : <SendedMessage key={iter}>{message.messageText}</SendedMessage>)
          )}
        </ChatBodySection>
